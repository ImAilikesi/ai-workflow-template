...
These are default working agreements for all repositories. Project-level `CLAUDE.md`,
`AGENTS.md`, or a pasted role prompt override these when more specific.
...
Use a handoff at a safe checkpoint when the current state, important decisions, constraints,
open findings, or next action cannot be recovered confidently without broad history
reconstruction.
...
## 11. Hybrid Workflow
Project `.claude/WORKFLOW.md` and its installed workflow skills own hybrid role topology,
provider boundaries, phase gates, review, continuation, and closure. Follow them when present;
do not reconstruct or duplicate their protocol in this global file.

Hybrid projects may use Claude for implementation and Codex-native roles for independent
review or bounded advisory work. The project workflow defines the exact models, authority,
routing, and gates.

If no project workflow defines additional mechanics, use the simplest execution path that
satisfies the request and these global rules.
...
- Context lifecycle and handoff are contractual: preserve a verified state and use the applicable
  workflow or handoff support before context uncertainty forces a guess.
...
## 13. Skills
Skills load from the active Claude environment and project skill surfaces.
...
Use project workflow skills such as `phase-gate` and `handoff` when their specific lifecycle
operation applies.
