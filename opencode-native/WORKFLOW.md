...
This file is the normative OpenCode phase contract. Project `AGENTS.md` defines project facts and
local rules. Skills define procedure. Custom subagents under `.opencode/agents/` define isolated
read-only roles. Do not duplicate this contract in project instructions.

## 1. Core model

Use one active OpenCode primary agent as phase owner and normal executor. Use custom subagents only
for bounded research, readiness, and independent review where isolated context adds value.

Normal work does not require this lifecycle. Use it when the accepted plan or operator selects a
phase gate.

The shipped custom subagents omit `model`. OpenCode therefore uses the model of the primary agent that
invoked them. The package does not pin a provider or model.
...
One OpenCode primary agent owns the accepted contract, implementation, integration, remediation,
verification, candidate freeze, and `CLOSE|REOPEN|BLOCK`.

Project custom subagents under `.opencode/agents/` provide these bounded roles:
...
The primary agent remains responsible for every accepted change.

### Read-only boundary

The shipped reviewer agents deny edit, bash, and nested task delegation. Their prompts also define
read-only authority. Readiness and review subagents must not edit, create, delete, rename, format,
commit, push, deploy, use secrets, or take destructive or external actions.

### Review lineage

OpenCode subagents run in child sessions. Preserve one logical review lineage in the parent with the
accepted contract, review round, material findings, resolved findings, candidate identity, and
recheck scope.

R1 uses one fresh independent-review child. Later rechecks should continue the same child session
when practical. If continuation is unavailable, invoke the same reviewer role with the explicit
review-state packet. Do not interpret a fresh child session as permission to run another full R1.

## 3. Phase sequence

1. Re-ground from project `AGENTS.md`, `.opencode/WORKFLOW.md`, the accepted phase contract, relevant
...
categories named above. Both omit `model`, so OpenCode inherits the invoking primary agent's model.
...
Use `.opencode/skills/opencode-handoff/SKILL.md` when a role can no longer recover its verified state
