...
- `live/pi/` — Pi global `AGENTS.md` only; do not ship a replacement system prompt by default.
- `live/omp/` — OMP native global `AGENTS.md` plus a minimal sticky `RULES.md`.
- `live/command-code/` — Command Code global `AGENTS.md` plus hand-maintained personal agents.
...
`sync.sh` targets the default native Pi and OMP roots (`~/.pi/agent` and `~/.omp/agent`). Do not auto-follow a shared `PI_CODING_AGENT_DIR` or OMP profile from this repository because two installed runtimes could otherwise collide into one managed destination.
...
Other harness global workflows use one lean model-agnostic shape: one primary parent owns orchestration and execution; independent read-only review is used only when it materially helps or the operator selects it; the same parent performs final verification and signoff. Project templates do not install or duplicate that workflow.

Pi uses only configured extension/agent surfaces for delegation. OMP should prefer its built-in reviewer over a duplicate custom reviewer. Command Code may use the tracked personal `independent-reviewer`; built-in `Explore` and `Plan` remain read-only support lanes.
...
Pi/OMP system prompts, runtime settings, profiles, credentials, package state, prompts, hooks, extensions, and MCP configuration stay out of this repository unless a future request verifies a specific public-safe editable surface and explicitly adds it. Command Code auth, provider settings, taste data, conversation history, and other machine-private state are also excluded.
